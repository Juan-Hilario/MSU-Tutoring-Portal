...
import frIcon from "../../assets/face-recognition.svg";
...
const Login: React.FC<LoginProps> = ({ setUser }) => {
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      ...
      if (
        fcWindowRef.current &&
        !fcWindowContentRef.current.contains(event.target)
      ) {
        closeWindow();
      }
    };
      ...
      <div className="login">
        ...
        <div className="loginContainer">
          <div className="loginFormSection">
            ...
            <h3>OR</h3>
            <button className="frButton" onClick={() => setFCWindow(!fcWindow)}>
              <img src={frIcon} width="50px" alt="" />
            </button>
          </div>
        </div>
